...
public class DashboardController {
    ...
    public Result<Map<String, Object>> getDashboardStats() {
        ...
        // 待审核项与今日统计：使用 audit-service 作为唯一权威来源
        long auditPending    = ((Number) aggregateStats.getOrDefault("pendingAudits",  0L)).longValue();
        long processedToday  = ((Number) aggregateStats.getOrDefault("processedToday",  0L)).longValue();
        long todayApproved   = ((Number) aggregateStats.getOrDefault("todayApproved",   0L)).longValue();

        aggregateStats.put("pendingAudits",  auditPending);
        aggregateStats.put("processedToday", processedToday);
        aggregateStats.put("todayApproved",  todayApproved);

        // 清除可能产生覆盖冲突或冗余的临时 key
